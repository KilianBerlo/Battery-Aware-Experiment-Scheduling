...
# States
class State(object):
	__slots__ = ("jobs_in_progress", "jobs_done", "time", "Worker_location", "Machine_location", "x")
	...
	def __eq__(self, other):
		return isinstance(other, self.__class__) and self.jobs_in_progress == other.jobs_in_progress and self.jobs_done == other.jobs_done and self.time == other.time and self.Worker_location == other.Worker_location and self.Machine_location == other.Machine_location and self.x == other.x
...
class WorkerAutomaton(object):
	...
	def __init__(self, network: Network):
		...
		self.transition_counts = [1, 1, 2, 1]
		self.transition_labels = [[1], [2], [3, 5], [4]]
		...
		if assignment_index == 0:
			...
			if location == 0:
				...
				if transition == 0:
					...
					if branch == 0:
						target_state.x = min((state.x + 1), 51)
						target_state.Machine_location = 1
...
class Network(object):
	...
	def __init__(self):
		self.network = self
		self.transition_labels = { 0: "τ", 1: "start_job", 2: "start_machine", 3: "machine_done", 4: "job_done", 5: "tick" }
		self.sync_vectors = [[0, -1, -1, -1, 0], [-1, 0, -1, -1, 0], [-1, -1, 0, -1, 0], [-1, -1, -1, 0, 0], [1, 1, -1, -1, 1], [4, 4, -1, -1, 4], [2, -1, 2, -1, 2], [3, -1, 3, -1, 3], [5, 5, 5, 5, 5]]
		self.properties = [Property("E_time", PropertyExpression("exists", [PropertyExpression("eventually", [PropertyExpression("ap", [0])])]))]
		self.variables = [VariableInfo("jobs_in_progress", None, "int", 0, 1), VariableInfo("jobs_done", None, "int", 0, 160), VariableInfo("time", None, "int", 0, 481), VariableInfo("Worker_location", 0, "int", 0, 3), VariableInfo("Machine_location", 2, "int", 0, 1), VariableInfo("x", 2, "int", 0, 51)]
	...
	def get_expression_value(self, state: State, expression: int):
		if expression == 0:
			return (((state.time == 480) and (state.jobs_in_progress == 0)) and (state.jobs_done >= 13))
		else:
			raise IndexError
