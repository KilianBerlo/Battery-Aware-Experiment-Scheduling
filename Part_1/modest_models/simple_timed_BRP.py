...
# States
class State(object):
	__slots__ = ("success", "failure", "coll", "Sender_location", "c", "n", "Channel_location", "c_1", "Channel_location_1", "c_2", "Receiver_location")
	...
	def __eq__(self, other):
		return isinstance(other, self.__class__) and self.success == other.success and self.failure == other.failure and self.coll == other.coll and self.Sender_location == other.Sender_location and self.c == other.c and self.n == other.n and self.Channel_location == other.Channel_location and self.c_1 == other.c_1 and self.Channel_location_1 == other.Channel_location_1 and self.c_2 == other.c_2 and self.Receiver_location == other.Receiver_location
...
class SenderAutomaton(object):
	...
	def __init__(self, network: Network):
		...
		self.transition_labels = [[1, 8], [2, 3, 8], [1, 0, 8], [8]]
		...
		if assignment_index == 0:
			...
			if location == 0:
				if transition == 0:
					...
					if branch == 0:
						target_state.c = min((state.c + 1), 11)
			elif location == 1:
				if transition == 0:
					if branch == 0:
...
class ChannelAutomaton(object):
	...
	def __init__(self, network: Network):
		...
		self.transition_labels = [[4, 4, 8], [0, 4, 8], [8], [5, 4, 8]]
		...
		if assignment_index == 0:
			...
			if location == 0:
				...
				if transition == 0:
					...
					if branch == 0:
						target_state.c_1 = min((state.c_1 + 1), 8)
			elif location == 3:
				if transition == 0:
					if branch == 0:
						target_state.c_1 = 0
		...
		if assignment_index == 0:
			...
			if location == 0:
				...
				if transition == 0:
					...
					if branch == 0:
						target_state.c_2 = min((state.c_2 + 1), 8)
			elif location == 3:
				if transition == 0:
					if branch == 0:
						target_state.c_2 = 0
...
class Network(object):
	...
	def __init__(self):
		self.network = self
		self.transition_labels = { 0: "τ", 1: "snd_data", 2: "rcv_ack", 3: "timeout", 4: "snd", 5: "rcv", 6: "rcv_data", 7: "snd_ack", 8: "tick" }
		self.sync_vectors = [[0, -1, -1, -1, 0], [-1, 0, -1, -1, 0], [-1, -1, 0, -1, 0], [-1, -1, -1, 0, 0], [3, -1, -1, -1, 3], [1, 4, -1, -1, 1], [2, -1, 5, -1, 2], [-1, 5, -1, 6, 6], [-1, -1, 4, 7, 7], [8, 8, 8, 8, 8]]
		...
		self.variables = [VariableInfo("success", None, "bool"), VariableInfo("failure", None, "bool"), VariableInfo("coll", None, "bool"), VariableInfo("Sender_location", 0, "int", 0, 3), VariableInfo("c", 0, "int", 0, 11), VariableInfo("n", 0, "int", 0, 2), VariableInfo("Channel_location", 1, "int", 0, 3), VariableInfo("c", 1, "int", 0, 8), VariableInfo("Channel_location", 2, "int", 0, 3), VariableInfo("c", 2, "int", 0, 8), VariableInfo("Receiver_location", 3, "int", 0, 1)]
